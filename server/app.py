...
def animal_by_id(id):
    ...
    zk = Zookeeper.query.filter(animal.zookeeper_id == Zookeeper.id).first()
    ec = Enclosure.query.filter(Enclosure.id == animal.enclosures_id ).first()
    ...
    response_body = f'''
        ...
        <ul>Zookeeper: {zk.name}</ul>
        <ul>Enclosure: {ec.environment}</ul>
    '''
